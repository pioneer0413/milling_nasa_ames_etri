...
- Feature-GRU는 80%에서 최저 RMSE를 보이지만, 배포 관점에서는 입력 신호 대부분을 기다려야 하므로 현실적인 조기 예측 지점으로 보기 어렵습니다.
- 특히 80%를 기준으로 좌우에서 RMSE가 다시 증가합니다(70%: 0.100585, 80%: 0.094549, 90%: 0.113379). 이는 단순히 더 긴 입력이 항상 유리한 구조가 아니라, 중간-후반 prefix에 성능 sweet spot이 있고 그 이후 late-stage 구간이 GRU에 노이즈/불안정을 추가할 수 있음을 시사합니다.

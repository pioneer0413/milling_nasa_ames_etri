#!/usr/bin/env python3
"""prj06 stacking — base = SVR/RandomForest/XGBoost, meta-learner = Ridge.

Same as prj05 but the meta-learner is Ridge (prj05 used MLP). Validation
identical to prj04/prj05: input 100%, leakage-free NESTED Leave-One-Case-Out,
repeated over 5 seeds; report mean±std. Feature = Raw+Delta+Meta(51).

  Outer fold (predict held-out case T):
    1) Inner LOCV over train cases -> OOF base predictions
    2) Train Ridge meta-learner on [OOF base preds] -> VB
    3) Refit base learners on all train cases, predict T
    4) Ridge meta-learner maps T's base preds -> final VB
...
def make_meta(seed):
    # Ridge meta-learner (input = 3 base preds)
...
def stack_predict(seqs, target, seed):
    ...
    return meta.predict(base_test), base_test.mean(axis=1), base_test, meta.coef_, meta.intercept_
...
def main():
    ...
    for c in sorted(rmse_cols, key=lambda c: overall[c]):
        ...
        tag = "  <-- STACK (Ridge meta)" if c == "stack_rmse" else (
    ...
    print("\nMean meta-learner (Ridge) coefficients (avg over seeds & folds):")
    for n in BASE_NAMES:

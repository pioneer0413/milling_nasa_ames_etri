...
Stacking with leakage-free NESTED Leave-One-Case-Out:
  Outer fold (predict held-out case T):
    1. Inner LOCV over the training cases (c != T): for each training case c,
       train the 5 base learners on (train cases except c) and predict c.
       -> out-of-fold (OOF) base predictions for every training run.
    2. Train the meta-learner on [OOF base preds] -> VB.
    3. Refit the 5 base learners on ALL training cases (!= T); predict T.
    4. Meta-learner maps T's base preds -> final VB.

Input length 100% only. Whole procedure repeated for 5 seeds; report mean±std.
Feature vector: Raw+Delta+Meta (51). Scaled (fit on train) for Ridge/SVR/MLP.

Meta-learner: Ridge(alpha=1.0). Also reports simple-average ensemble and each
standalone base learner for comparison, plus mean meta-learner coefficients.

Outputs: cache/per_case.csv, cache/summary.csv, cache/meta_coef.csv
"""
...
PREFIX        = 100
SEEDS         = [0, 1, 2, 3, 4]
BASE_NAMES    = ["Ridge", "SVR", "RandomForest", "XGBoost", "MLP"]
SCALE_MODELS  = {"Ridge", "SVR", "MLP"}
...
def make_base(seed):
    return {
        ...
        "MLP":          MLPRegressor(hidden_layer_sizes=(64, 32), max_iter=1500,
...
def fit_base(Xtr, ytr, seed):
    ...
    Xs = sc.transform(Xtr)
    fitted = {}
    for name, mdl in make_base(seed).items():
        fitted[name] = mdl.fit(Xs if name in SCALE_MODELS else Xtr, ytr)
    return fitted, sc
...
# ── nested stacking for one (test case, seed) ────────────────────────────────

def stack_predict(seqs, target, seed):
    train_ids = [c for c in CASE_SCOPE if c != target and c in seqs]
    # 1) inner LOCV -> OOF base predictions over training cases
    oof_P, oof_y = [], []
    for c in train_ids:
    ...
    # 2) meta-learner on OOF base preds
    meta = Ridge(alpha=1.0).fit(oof_P, oof_y)
    # 3) refit base on ALL training cases, predict target
    ...
    base_test = predict_base(fit_full, sc_full, seqs[target]["X"])     # (n_runs, 5)
    # 4) final predictions
    stack_pred = meta.predict(base_test)
    avg_pred   = base_test.mean(axis=1)
    return stack_pred, avg_pred, base_test, meta.coef_, meta.intercept_
...
def main():
    ...
    print(f"features={len(FEAT_COLS)} (Raw24+Delta24+Meta3)  cases={len(seqs)}  "
          f"base={BASE_NAMES}  meta=Ridge  seeds={SEEDS}\n", flush=True)

    def one_fold(seed, tc):
        sp, ap, base_test, coef, intc = stack_predict(seqs, tc, seed)
        yt = seqs[tc]["vb"]
        ...
        crec = {"seed": seed, "target_case": tc, "intercept": float(intc),
                **{f"coef_{n}": float(coef[i]) for i, n in enumerate(BASE_NAMES)}}
        return rec, crec

    jobs = [(seed, tc) for seed in SEEDS for tc in CASE_SCOPE if tc in seqs]
    print(f"Nested-stacking folds: {len(jobs)} (parallel)\n", flush=True)
    out = Parallel(n_jobs=-1, verbose=5)(delayed(one_fold)(s, t) for s, t in jobs)
    rows  = [r for r, _ in out]
    coefs = [c for _, c in out]
    for seed in SEEDS:
    ...
    cdf = pd.DataFrame(coefs); cdf.to_csv(CACHE/"meta_coef.csv", index=False)

    rmse_cols = ["stack_rmse", "avg_rmse"] + [f"{n}_rmse" for n in BASE_NAMES]
    fold_means = df.groupby("seed")[rmse_cols].mean()      # per-seed 15-fold mean
    overall = fold_means.mean(); overall_std = fold_means.std()

    print("\n" + "="*64)
    print("OVERALL 15-fold LOCV RMSE @100%  (mean ± std over 5 seeds)")
    print("="*64)
    order = sorted(rmse_cols, key=lambda c: overall[c])
    for c in order:
        label = c.replace("_rmse", "")
        tag = "  <-- STACK (meta-learner)" if c == "stack_rmse" else (
    ...
    print("\nMean meta-learner coefficients (avg over seeds & folds):")
    for n in BASE_NAMES:
        print(f"  {n:14s}: {cdf[f'coef_{n}'].mean():+.3f}")
    print(f"  intercept     : {cdf['intercept'].mean():+.3f}")
    print(f"\nSaved: cache/per_case.csv, cache/summary.csv, cache/meta_coef.csv")

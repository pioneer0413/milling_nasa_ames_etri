#!/usr/bin/env python3
"""H4_S3_T2: Segment-Based Feature Extraction — Entry+Steady only

Context spec: docs/agent_context/task_context/H4_S3_T2_context.md

Segment: signal[idx_noload_end : idx_end]  (Entry + Steady)
Features: Raw(24) + Delta(24) + Meta(3) = 51 tabular; Delta+Meta(27) GRU

Output:
  experiments/executions/H4/S3/T2/{timestamp}_H4_S3_T2_entry_steady/
"""
...
PREFIX_LABEL  = "H4_S3_T2"
TOPIC         = "entry_steady"
THRESH        = 1e6
META_COLS     = ["DOC", "feed", "material"]
TABULAR_MODELS = ["ridge", "random_forest", "xgboost", "svr", "mlp"]

# segment slice: Entry+Steady
def get_slice(row):
    return int(row.idx_noload_end), int(row.idx_end)
...
def main():
    ...
    out = ROOT/"experiments"/"executions"/"H4"/"S3"/"T2"/f"{ts}_H4_S3_T2_{TOPIC}"

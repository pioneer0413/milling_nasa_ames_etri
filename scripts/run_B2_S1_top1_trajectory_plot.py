#!/usr/bin/env python3
"""B2_S1 (addendum): VB Prediction Trajectory — FeatGRU / FeatLSTM top-1 (leaderboard)

leaderboard.csv(prefix 제외) 기준 feature_gru/feature_lstm 각각의 top-1 설정으로
LOCV 예측값을 뽑아 case별 VB trajectory(실측 vs 예측)를 시각화한다.

  FeatGRU  top1: AC+vT+vS, Delta+Meta(15-dim), Full segment, RMSE=0.095122
  FeatLSTM top1: AC+vT+vS, Delta+Meta(15-dim), Full segment, RMSE=0.092217

(둘 다 [B2_S1] 2026-06-22 결과와 동일 설정 — mask=13, segment=Full)

Output: experiments/executions/B2/S1/{timestamp}_top1_trajectory/
"""
...
MASK          = 13   # AC+vT+vS — top1 config for both FeatGRU and FeatLSTM
MODELS        = {"FeatGRU": 0.095122, "FeatLSTM": 0.092217}   # model -> leaderboard top1 rmse (ref)
SEED          = 0   # representative seed for trajectory visualization
...
def run_locv_predictions(all_cases, input_dim, cell_type, device, seed):
    preds: dict[int, dict] = {}
    for tc in CASE_SCOPE:
        ...
        with warnings.catch_warnings():
            ...
            out = fit_predict_rnn(train, {tc: all_cases[tc]}, input_dim, cell_type, device, seed)
        preds[tc] = {
            "run": all_cases[tc]["runs"],
...
# ─── Plot ─────────────────────────────────────────────────────────────────────
def plot_trajectories(model_type: str, preds: dict, rmse_ref: float, out_dir: Path) -> None:
    ...
    fig.suptitle(f"{model_type} — leaderboard top1 (AC+vT+vS, Full, LOCV RMSE={rmse_ref:.6f})", fontsize=13)
    plt.tight_layout()
    fname = f"trajectory_top1_{model_type}"
...
def main() -> None:
    ...
    out_dir = ROOT / "experiments" / "executions" / "B2" / "S1" / f"{ts}_top1_trajectory"
    ...
    print(f"Clean runs: {len(proc_clean)}, cache size: {len(raw_cache)}")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    input_dim = bin(MASK).count("1") * 4 + len(META_FEATURES)

    for model_type, rmse_ref in MODELS.items():
        print(f"\n{model_type}: running LOCV predictions (seed={SEED})...")
        preds = run_locv_predictions(all_cases, input_dim, model_type, device, SEED)
        plot_trajectories(model_type, preds, rmse_ref, out_dir)
        print(f"  Saved trajectory_top1_{model_type}.png")

#!/usr/bin/env python3
"""H4_S1_T2: Input Signal Length Effect — Raw+Delta+Meta Features for All Models

Context spec: docs/agent_context/task_context/H4_S1_T2_context.md
Scenario:     docs/agent_context/scenario_context/H4_S1_context.md
Change vs T1: ALL models use Raw + Delta + Meta features (T1: tabular=Raw+Meta only)

Feature vector per sample (51 features):
  Raw   (24): mean/rms/std/peak × 6 sensors  — signal statistics at prefix
  Delta (24): Δmean/Δrms/Δstd/Δpeak × 6 sensors — difference from Run1 stats
  Meta   (3): DOC, feed, material

Output:
  experiments/executions/H4/S1/{timestamp}_T2_raw_delta_meta_all_models/
    configs/H4_S1_T2_input_config.json
    metrics/H4_S1_T2_shift_metrics.csv
    analysis/H4_S1_T2_locv_aggregated.csv
    analysis/H4_S1_T2_100pct_per_case.csv
    logs/H4_S1_T2_run_log.txt
"""
...
TOPIC = "T2_raw_delta_meta_all_models"
PREFIX_LABEL = "H4_S1_T2"
META_FEATURES = ["DOC", "feed", "material"]
TABULAR_MODELS = ["ridge", "random_forest", "xgboost", "svr", "mlp"]
THRESH = 1e6  # signal anomaly threshold
...
def extract_features(arr: np.ndarray) -> np.ndarray:
    """Returns [mean, rms, std, peak] for one sensor signal prefix."""
...
def build_feature_matrix(
    ...
    """
    Builds feature matrix with columns: raw (24) + delta (24) + meta (3) = 51.
    Delta = current_run_stats - first_available_run_stats (within each case × prefix_pct).
    """
    raw_cols = raw_col_names()
    delta_cols = delta_col_names()

    # Pre-compute raw features for every (case, run, pct)
    cache: dict[tuple[int, int, int], np.ndarray] = {}  # (case, run, pct) -> (24,)
    ...
    # Identify first available run per case (after exclusion)
    first_run: dict[int, int] = {}
    for case_id in process_df["case"].unique():
        runs_in_case = sorted(
            r for (c, r, _) in cache if c == case_id and (c, r) not in EXCLUDED_RUNS
        )
        if runs_in_case:
            first_run[int(case_id)] = runs_in_case[0]
...
# ─── GRU for T2: Raw + Delta + Meta sequence ─────────────────────────────────
...
def build_gru_sequences(
    ...
    """Build per-case sequences with [raw, delta, meta] features at each run step."""
...
def run_locv(
    ...
    if model_name == "feature_gru":
        ...
        for test_case in CASE_SCOPE:
            ...
            results.append({"signal_prefix_percent": pct, "model": model_name, "seed": SEED, "target_case": test_case, "n_runs": len(y_true), **m})
        return results

    # Tabular models: use StandardScaler on full 51-feature set
    for test_case in CASE_SCOPE:
...
def main() -> None:
    ...
    log("=== H4_S1_T2 Experiment start (Raw+Delta+Meta for all models) ===")
    log(f"Output dir: {output_dir}")

    # Load
    log("Loading data...")
    signal_df = pd.read_csv(ROOT / "datasets/processed/mill_signal_data.csv", usecols=["case", "run"] + SENSORS)
    ...
    log(f"After preprocessing: {len(process_clean)} rows (excluded Case2-Run1, Case12-Run1)")

    # Build feature matrix (Raw + Delta + Meta)
    log("Building Raw+Delta+Meta feature matrix...")
    raw_cols = raw_col_names()
    delta_cols = delta_col_names()
    feat_df = build_feature_matrix(signal_df, process_clean, PREFIX_PERCENTS, SENSORS)
    log(f"Feature matrix: {len(feat_df)} rows, {len(raw_cols)+len(delta_cols)+len(META_FEATURES)} total features (raw={len(raw_cols)}, delta={len(delta_cols)}, meta={len(META_FEATURES)})")

    # Save config
    config = {
        "experiment": {"id": f"{timestamp}_{PREFIX_LABEL}", "context": "docs/agent_context/task_context/H4_S1_T2_context.md"},
        "change_vs_T1": "All models use Raw+Delta+Meta features (T1: tabular=Raw+Meta, GRU=Delta+Meta)",
        "features": {"raw_features": 24, "delta_features": 24, "meta_features": 3, "total": 51},
        "prefix_percents": PREFIX_PERCENTS,
        "models": TABULAR_MODELS + ["feature_gru"],
        "seed": SEED,
        "split": "leave_one_case_out",
    }
    (output_dir / "configs" / "H4_S1_T2_input_config.json").write_text(json.dumps(config, indent=2), encoding="utf-8")
    ...
    agg = (
        metrics_df.groupby(["signal_prefix_percent", "model"])

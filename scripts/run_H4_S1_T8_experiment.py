#!/usr/bin/env python3
"""H4_S1_T8: Input Signal Length Effect — Raw+Delta Features (No Meta)

Context spec: docs/agent_context/task_context/H4_S1_T8_context.md

Feature vectors:
  Tabular: Raw(24) + Delta(24) = 48 features
  GRU:     Raw(24) + Delta(24) = 48 dims per run step

Output:
  experiments/executions/H4/S1/T8/{timestamp}_H4_S1_T8_raw_delta_no_meta/
"""
...
PREFIX_LABEL    = "H4_S1_T8"
TOPIC           = "raw_delta_no_meta"
THRESH          = 1e6
...
def all_feat_cols():
    return raw_col_names() + delta_col_names()
...
def build_feature_matrix(signal_df, process_df, prefix_percents, sensors):
    ...
    # first-available-run reference per case per pct
    first_run_cache = {}
    for pct in prefix_percents:
...
def build_sequences(feature_df, feat_cols, pct):
...
def fit_predict_gru(train_cases, test_cases, input_dim, device, epochs=200, lr=0.001):
    ...
    model = RawDeltaGRU(input_dim).to(device)
...
def run_locv(feature_df, feat_cols, pct, model_name, device):
    ...
    if model_name == "feature_gru":
        case_data = build_sequences(feature_df, feat_cols, pct)
...
def main():
    ...
    out = ROOT/"experiments"/"executions"/"H4"/"S1"/"T8"/f"{ts}_H4_S1_T8_{TOPIC}"
    ...
    log("=== H4_S1_T8: Raw+Delta (no meta) ===")
    ...
    log(f"Feature matrix: {len(feat_df)} rows, {len(feat_cols)} features")
    ...
    (out/"configs"/f"{PREFIX_LABEL}_input_config.json").write_text(
        json.dumps({"experiment": {"id": f"{ts}_{PREFIX_LABEL}",
                    ...
                    "features": {"tabular": "Raw(24)+Delta(24)", "gru": "Raw(24)+Delta(24)"},
    ...
    for mn in TABULAR_MODELS + ["feature_gru"]:
        ...
        for pct in PREFIX_PERCENTS:
            try:
                all_results.extend(run_locv(feat_df, feat_cols, float(pct), mn, device))

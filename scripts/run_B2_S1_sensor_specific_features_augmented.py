#!/usr/bin/env python3
"""B2_S1 (variant): All-sensors FeatGRU with 4-stat + sensor-specific 2-stat (augmented)

기존 4-stat(mean/rms/std/max, 전 센서 동일) 유지 + 센서 유형별 2개 feature 추가
(대체가 아니라 augmentation):

  smcAC, smcDC (전류)        : + skewness, slope(선형추세)
  vib_table/spindle (진동)    : + crest_factor, spectral_centroid
  AE_table/spindle (음향방출)  : + kurtosis, burst_count(|x-mean|>3*std 비율)

센서당 6-stat x 6센서 = 36 raw-dim, Delta+Meta = 36+3 = 39-dim.
FeatGRU만 수행(사용자 지시).

비교 대상:
  all6 4-stat(24-dim)              : FeatGRU=0.114839
  AC+vT+vS 4-stat(15-dim)          : FeatGRU=0.095122
  all6 Hampel-smoothed             : FeatGRU=0.140917 (negative)
  센서맞춤 2-stat 단독 교체(15-dim) : FeatGRU=0.175981 (negative, 최악)

Output: experiments/executions/B2/S1/{timestamp}_sensor_specific_augmented/
"""
...
SEEDS         = [0, 1, 2, 3, 4]
THRESH        = 1e6
N_STATS_PER_SENSOR = 6   # 4 base + 2 sensor-specific
MODELS        = ["FeatGRU"]

REF = {
    "all6_4stat":        0.114839,
    "ACvTvS_4stat":      0.095122,
    "all6_hampel":       0.140917,
    "sensor_specific_2": 0.175981,
}
...
# ─── Feature extraction ───────────────────────────────────────────────────────
def base_4stat(arr: np.ndarray) -> np.ndarray:
    return np.array([np.mean(arr), np.sqrt(np.mean(arr ** 2)), np.std(arr), np.abs(arr).max()])


def extra_2stat(sensor: str, arr: np.ndarray) -> np.ndarray:
    n = arr.size
    if sensor in CURRENT_SENSORS:
        skewness = scipy_skew(arr, bias=True) if n > 3 else 0.0
        x = np.arange(n, dtype=np.float64)
        slope = np.polyfit(x, arr, 1)[0] if n > 1 else 0.0
        return np.array([_safe(skewness), _safe(slope)])
    ...
    if sensor in VIB_SENSORS:
        ...
        centroid = float(np.sum(freqs * spec) / np.sum(spec)) if np.sum(spec) > 1e-12 else 0.0
        return np.array([_safe(crest), _safe(centroid)])
    # AE_SENSORS
    kurt = scipy_kurtosis(arr, fisher=True, bias=True) if n > 3 else 0.0
    std = np.std(arr)
    burst = float(np.mean(np.abs(arr - np.mean(arr)) > 3 * std)) if std > 1e-12 else 0.0
    return np.array([_safe(kurt), _safe(burst)])
...
def extract_features_for_sensor(sensor: str, arr: np.ndarray) -> np.ndarray:
    ...
    if arr.size < 4:
        return np.zeros(N_STATS_PER_SENSOR, dtype=np.float64)
    return np.concatenate([base_4stat(arr), extra_2stat(sensor, arr)])
...
def build_raw_cache(signal_df: pd.DataFrame, proc_clean: pd.DataFrame):
    ...
    for row in proc_clean.itertuples(index=False):
        ...
        cache[(case_id, run_id)] = full   # 36-dim (6 sensors x 6 stats)
...
def main() -> None:
    ...
    out_dir = ROOT / "experiments" / "executions" / "B2" / "S1" / f"{ts}_sensor_specific_augmented"
    ...
    log("=== B2_S1 variant: 4-stat + sensor-specific 2-stat augmented (FeatGRU only) ===")
    log("Current(+skew,+slope) / Vibration(+crest,+spec_centroid) / AE(+kurtosis,+burst_count)")
    ...
    log(f"Input dim: {input_dim} (36 augmented delta + 3 meta)")
    ...
    for m in MODELS:
        r = results[m]["mean"]
        log(f"{m}: augmented={r:.6f}")
        for name, ref in REF.items():
            d = r - ref
            log(f"  vs {name:<20} ref={ref:.6f}  Δ={d:+.6f} ({'개선' if d<0 else '악화'})")

    pd.DataFrame([
        {"model": m, "mean": r["mean"], "std": r["std"], **{f"ref_{k}": v for k, v in REF.items()}}
        for m, r in results.items()
    ]).to_csv(out_dir / "metrics" / "augmented_results.csv", index=False)

    fig, ax = plt.subplots(figsize=(9, 5))
    names = ["FeatGRU\naugmented(39d)"] + [f"FeatGRU\n{k}" for k in REF]
    vals = [results["FeatGRU"]["mean"]] + list(REF.values())
    colors = ["#d65f5f", "#4878d0", "#6acc65", "#ee854a", "#8c613c"]
    ...
    ax.set_title("FeatGRU: 4-stat+augmented(2/sensor) vs prior variants")
    ax.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    fig.savefig(str(out_dir / "figures" / "augmented_comparison.png"), dpi=150, bbox_inches="tight")
    plt.close(fig)

    summary = {
        "experiment": "B2_S1_sensor_specific_augmented",
        "feature_scheme": {
            "base": ["mean", "rms", "std", "max"],
            "current_extra": ["skewness", "slope"],
            "vibration_extra": ["crest_factor", "spectral_centroid"],
            "AE_extra": ["kurtosis", "burst_count"],
        },
        "input_dim": input_dim, "results": results, "references": REF,

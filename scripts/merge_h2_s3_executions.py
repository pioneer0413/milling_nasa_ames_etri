...
from milling_experiment_framework.experiments.h2_s3_feature_combination_execution import H2S3FeatureCombinationExecution
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Merge H2.S3 initial and extra seed executions into one 20-seed result.")
    ...
    experiment_id = (
        ...
        + "_H2_S3_feature_combination_all_sensors_segment_aware_VB_prediction_seeds_0_19_combined"
    )
    paths = ExperimentPaths(root, experiment_id)
    paths.prepare_standard_dirs()

    helper = H2S3FeatureCombinationExecution(args.config, root=root, dry_run=False, seed_mode="all")
    ...
    contribution = helper._feature_group_contribution(combo_metrics)
    combo_effect = helper._feature_combination_effect(combo_metrics)
    best_combo = helper._best_feature_combination(combo_metrics)
    ...
    baseline = pd.concat(
        [
            combo_effect.assign(comparison_type="feature_combination"),
    ...
    write_csv(paths.execution_dir / "metrics" / "feature_combination_metrics.csv", combo_metrics)
    ...
    write_csv(paths.execution_dir / "analysis" / "feature_group_contribution.csv", contribution)
    write_csv(paths.execution_dir / "analysis" / "feature_combination_effect.csv", combo_effect)
    write_csv(paths.execution_dir / "analysis" / "best_feature_combination.csv", best_combo)
    write_csv(paths.execution_dir / "analysis" / "segment_effect_under_feature_combination.csv", segment_effect)
    write_csv(paths.execution_dir / "analysis" / "best_segment_by_feature_combination.csv", best_segment)
    ...
    (paths.execution_dir / "logs" / "run.log").write_text(
        "\n".join(
            [
                f"{datetime.now().isoformat(timespec='seconds')} INFO H2.S3 merged execution started: {experiment_id}",
                ...
                f"{datetime.now().isoformat(timespec='seconds')} INFO H2.S3 merged execution finished: {experiment_id}",
                "",
            ]
        ),
        encoding="utf-8",

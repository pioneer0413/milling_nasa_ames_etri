...
[LEGACY] Notion [B3] Naive Baseline의 최초 구현. 현재 B3 최신본은
run_B3_S1_comprehensive_baseline.py(13-model 종합, 본 스크립트의 CF/RL/ML/DL 포함)이며
이 파일은 superseded. 파일명은 이력 보존을 위해 H18로 유지(rename 대상에서 제외).
...
Reference (from B4_S1 at 100% input, 5-seed):
...
# Reference from B4_S1 (5-seed, 100% input)
REF = {
    "GRU AC+vT+vS (B4)": 0.095122,
    "XGB AC+vS    (B4)": 0.109239,
}
...
def main() -> None:
    ...
    for k, rv in REF.items():
        agg_rows.append({"model": k, "rmse": rv, "description": "reference from B4_S1"})

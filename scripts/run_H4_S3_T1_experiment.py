#!/usr/bin/env python3
"""H4_S3_T1: Segment-Based Feature Extraction — Entry+Steady+Exit (No-load excluded)

Context spec: docs/agent_context/task_context/H4_S3_T1_context.md

Segment: signal[idx_noload_end : signal_length]  (Entry + Steady + Exit)
Features: Raw(24) + Delta(24) + Meta(3) = 51 tabular; Delta+Meta(27) GRU

Output:
  experiments/executions/H4/S3/T1/{timestamp}_H4_S3_T1_entry_steady_exit/
"""
...
PREFIX_LABEL  = "H4_S3_T1"
TOPIC         = "entry_steady_exit"
THRESH        = 1e6
META_COLS     = ["DOC", "feed", "material"]
TABULAR_MODELS = ["ridge", "random_forest", "xgboost", "svr", "mlp"]

# segment slice: Entry+Steady+Exit (no-load removed)
def get_slice(row):
    return int(row.idx_noload_end), None  # None = to end of signal
...
def build_feature_matrix(signal_df, proc_df, meta_df):
    """proc_df already merged with meta_df (contains idx_noload_end etc.)"""
    ...
    for row in proc_df.itertuples(index=False):
        ...
        sliced = {s: (arrays[s][sl_start:sl_end] if sl_end else arrays[s][sl_start:]) for s in SENSORS}
        feats = np.concatenate([extract_features(sliced[s]) for s in SENSORS])
        raw_cache[(cid, rid)] = feats
    # first available run reference per case
    first_cache = {}
...
def main():
    ...
    out = ROOT/"experiments"/"executions"/"H4"/"S3"/"T1"/f"{ts}_H4_S3_T1_{TOPIC}"
    ...
    # inner join: only runs with heuristic metadata

#!/usr/bin/env python3
"""B2_S2: Ablation: Segment — Entry / Steady / Entry_Steady x {FeatGRU, FeatLSTM, XGBoost, RandomForest}

[B2] 명세(Notion) 잔여 segment 실행. B2_S1(Full/Exclude_exit)에 이어 idx_start 의존
segment 3종을 실행한다.

⚠️ PROVISIONAL: idx_start(steady 경계) 오염 이슈가 [B1](2026-06-23)에서 확인됐고
아직 미해결 상태다. 본 결과는 idx_start를 직접 경계로 사용하므로 잠정치이며,
오염 이슈가 해결되면 재실행이 필요할 수 있다. 사용자 명시 지시로 실행.

Segment 정의 (cutting_segment_index.csv 컬럼 기준, [B2] 단일화 결정과 동일):
  Entry         : [idx_noload_end, idx_start)
  Steady        : [idx_start, idx_exit_start)
  Entry_Steady  : [idx_noload_end, idx_exit_start)

피처: Delta+Meta (All sensors, mask=63, 27-dim) — 2026-06-22 사용자 지시로 B2_S1과 함께 갱신,
센서 subset 대신 전체 6센서를 4종 모델에 동일 적용.
평가: LOCV 15 cases, observed-VB-only RMSE.
  - 확률적 모델(FeatGRU/FeatLSTM/RandomForest): 5-seed mean±std
  - 결정론적 모델(XGBoost): 1회 실행

Output: experiments/executions/B2/S2/{timestamp}_segment_ablation_remaining/
"""
...
SEGMENTS      = ["Entry", "Steady", "Entry_Steady"]   # PROVISIONAL — idx_start contamination unresolved
MODELS        = ["FeatGRU", "FeatLSTM", "XGBoost", "RandomForest"]
STOCHASTIC    = {"FeatGRU", "FeatLSTM", "RandomForest"}

# B2_S1 reference (idx_start-independent, reliable), All sensors mask=63, rerun 2026-06-22
REF_FULL = {"FeatGRU": 0.114839, "FeatLSTM": 0.117919, "XGBoost": 0.113095, "RandomForest": 0.135995}
REF_EXCL_EXIT = {"FeatGRU": 0.115662, "FeatLSTM": 0.109700, "XGBoost": 0.110914, "RandomForest": 0.135570}
...
def segment_bounds(segment: str, idx_noload_end: int, idx_start: int, idx_exit_start: int, base_len: int) -> tuple[int, int]:
    if segment == "Entry":
        start, end = idx_noload_end, idx_start
    elif segment == "Steady":
        start, end = idx_start, idx_exit_start
    elif segment == "Entry_Steady":
        start, end = idx_noload_end, idx_exit_start
    else:
        raise ValueError(f"Unknown segment: {segment}")
    start = max(0, min(start, base_len))
    end   = max(0, min(end, base_len))
    if end <= start:
        end = min(start + 1, base_len)
        start = max(0, end - 1)
    return start, end
...
def build_raw_cache(
    ...
    seg_lookup = {
        (int(r.case), int(r.run)): (int(r.idx_noload_end), int(r.idx_start), int(r.idx_exit_start))
    ...
    for row in proc_clean.itertuples(index=False):
        ...
        bounds = seg_lookup.get((case_id, run_id))
        if bounds is None:
            continue
        idx_noload_end, idx_start, idx_exit_start = bounds
        start, end = segment_bounds(segment, idx_noload_end, idx_start, idx_exit_start, base_len)

        full = np.concatenate([extract_features(arrays[s][start:end]) for s in SENSORS])
...
def plot_results(results: dict[str, dict[str, dict]], out_dir: Path) -> None:
    all_segments = ["Full", "Exclude_exit"] + SEGMENTS
    fig, ax = plt.subplots(figsize=(13, 5.5))
    x = np.arange(len(all_segments))
    width = 0.2
    colors = {"FeatGRU": "#4878d0", "FeatLSTM": "#ee854a", "XGBoost": "#6acc65", "RandomForest": "#d65f5f"}
    for i, model_type in enumerate(MODELS):
        means = [REF_FULL[model_type], REF_EXCL_EXIT[model_type]] + \
                [results[model_type][seg]["mean"] for seg in SEGMENTS]
        stds  = [0.0, 0.0] + [results[model_type][seg]["std"] for seg in SEGMENTS]
        ax.bar(x + (i - 1.5) * width, means, width, yerr=stds, capsize=3,
               color=colors[model_type], label=model_type)
    ax.axvspan(1.5, len(all_segments) - 0.5, alpha=0.08, color="red")
    ax.set_xticks(x)
    ax.set_xticklabels(all_segments)
    ax.set_ylabel("Observed-VB RMSE")
    ax.set_title("B2_S1+S2: Segment Ablation (shaded = PROVISIONAL, idx_start contamination unresolved)")
    ax.legend(fontsize=9)
    ax.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    fig.savefig(str(out_dir / "segment_ablation_remaining.png"), dpi=150, bbox_inches="tight")
    fig.savefig(str(out_dir / "segment_ablation_remaining.svg"), bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    out_dir = ROOT / "experiments" / "executions" / "B2" / "S2" / f"{ts}_segment_ablation_remaining"
    ...
    log("=== B2_S2: Segment Ablation — Entry / Steady / Entry_Steady ===")
    log("WARNING: PROVISIONAL — idx_start contamination unresolved ([B1], 2026-06-23). Run per explicit user instruction.")
    ...
    # ── Save CSVs ─────────────────────────────────────────────────────────────
    agg_rows = [
        {"model": m, "segment": s, "mean": r["mean"], "std": r["std"], "n_seed": r["n_seed"], "provisional": True}
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    log("\n=== SUMMARY (PROVISIONAL — idx_start contamination unresolved) ===")
    log(f"{'Model':<14} {'Full(ref)':>10} {'Excl_exit(ref)':>15} {'Entry':>10} {'Steady':>10} {'Entry_Steady':>14}")
    log("-" * 78)
    for m in MODELS:
        log(f"{m:<14} {REF_FULL[m]:>10.6f} {REF_EXCL_EXIT[m]:>15.6f} "
            f"{results[m]['Entry']['mean']:>10.6f} {results[m]['Steady']['mean']:>10.6f} "
            f"{results[m]['Entry_Steady']['mean']:>14.6f}")

    best_overall_all = min(
        [(m, "Full", REF_FULL[m]) for m in MODELS] +
        [(m, "Exclude_exit", REF_EXCL_EXIT[m]) for m in MODELS] +
        [(m, s, results[m][s]["mean"]) for m in MODELS for s in SEGMENTS],
        key=lambda t: t[2],
    )
    log(f"\nBest overall across ALL 5 segments (incl. provisional): "
        f"{best_overall_all[0]}/{best_overall_all[1]}  mean={best_overall_all[2]:.6f}")

    summary = {
        "experiment": "B2_S2_segment_ablation_remaining",
        "provisional": True,
        "provisional_reason": "idx_start contamination unresolved, see [B1] 2026-06-23",
        "mask": MASK, "sensor_subset": mask_label(MASK),
        "models": MODELS, "segments": SEGMENTS, "seeds": SEEDS,
        "results": results,
        "ref_full": REF_FULL, "ref_exclude_exit": REF_EXCL_EXIT,
        "best_overall_all_segments": {"model": best_overall_all[0], "segment": best_overall_all[1],
                                       "mean": best_overall_all[2]},

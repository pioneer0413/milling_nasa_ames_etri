...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run H3_S3_T1 steady-length model prediction effect experiment.")
...
def execution_dir(output_root: Path, timestamp: str) -> Path:
    return output_root / "H3" / "S3" / "T1" / f"{timestamp}_{TOPIC}"
...
def build_config(args: argparse.Namespace, timestamp: str, output_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "features": {
            "source": "H3_S2_T1 length-specific steady-cut feature extraction",
...
def plot_length_effect(output_dir: Path, model_metrics: pd.DataFrame, dpi: int) -> Path:
    ...
    fig.tight_layout()
    path = output_dir / "figures" / "H3_S3_T1_length_effect_rmse.png"
...
def write_report(output_dir: Path, summary: dict[str, Any], model_metrics: pd.DataFrame, best_by_model: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Outputs",
        "",
        "- Feature matrix: `data/H3_S3_T1_feature_matrix.csv`",
        "- Common preprocessing: `analysis/H3_S3_T1_common_preprocessing.json`",
        "- Shift metrics: `metrics/H3_S3_T1_shift_metrics.csv`",
        "- Seed metrics: `metrics/H3_S3_T1_seed_metrics.csv`",
        "- Length-model metrics: `metrics/H3_S3_T1_length_model_metrics.csv`",
        "- Predictions: `predictions/H3_S3_T1_predictions.csv`",
        "- Length effect figure: `figures/H3_S3_T1_length_effect_rmse.{png,svg}`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Method Note",
            "",
            "For each steady-cut length, the selected H1/H3 features are recomputed per selected sensor from the corresponding steady metadata window. "
    ...
    (output_dir / "reports" / "H3_S3_T1_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_yaml(output_dir / "configs" / "H3_S3_T1_input_config.yaml", config)
    write_json(output_dir / "logs" / "H3_S3_T1_environment.json", collect_environment())
    ...
    feature_wide.to_csv(output_dir / "data" / "H3_S3_T1_feature_matrix.csv", index=False)
    write_json(output_dir / "analysis" / "H3_S3_T1_common_preprocessing.json", preprocessing_report)
    if args.dry_run:
        write_json(output_dir / "analysis" / "H3_S3_T1_summary.json", summary)
        return summary
    ...
    shift_metrics.to_csv(output_dir / "metrics" / "H3_S3_T1_shift_metrics.csv", index=False)
    seed_metrics.to_csv(output_dir / "metrics" / "H3_S3_T1_seed_metrics.csv", index=False)
    model_metrics.to_csv(output_dir / "metrics" / "H3_S3_T1_length_model_metrics.csv", index=False)
    best_by_model.to_csv(output_dir / "analysis" / "H3_S3_T1_best_length_by_model.csv", index=False)
    predictions.to_csv(output_dir / "predictions" / "H3_S3_T1_predictions.csv", index=False)
    splits.to_csv(output_dir / "splits" / "H3_S3_T1_splits.csv", index=False)
...
def main() -> None:
    ...
    try:
        ...
        if output_dir is not None:
            (output_dir / "logs" / "H3_S3_T1_error.log").write_text(traceback.format_exc(), encoding="utf-8")
        raise

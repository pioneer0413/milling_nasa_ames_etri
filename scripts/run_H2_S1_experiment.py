...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run S1 segment-setting effect on VB prediction.")
    ...
    result = S1SegmentExecution(args.config, root=Path.cwd(), dry_run=args.dry_run, seed_mode=args.seed_mode, assume_yes=args.yes).run()
    print(result)

#!/usr/bin/env python3
"""H4_S3_T4: No-load Excluded + Input Ratio 10~100%

Context spec: docs/agent_context/task_context/H4_S3_T4_context.md

Signal slice per ratio:
  cutting_signal = signal[idx_noload_end : signal_length]
  used_signal    = cutting_signal[:ceil(len(cutting_signal) * ratio / 100)]

Features: Raw(24) + Delta(24) + Meta(3) = 51 tabular; Delta+Meta(27) GRU

Output:
  experiments/executions/H4/S3/T4/{timestamp}_H4_S3_T4_noload_excluded_prefix/
"""
...
INPUT_RATIOS   = list(range(10, 101, 10))
SEED           = 0
EXCLUDED_RUNS  = {(2, 1), (12, 1)}
PREFIX_LABEL   = "H4_S3_T4"
TOPIC          = "noload_excluded_prefix"
THRESH         = 1e6
...
def build_feature_matrix(signal_df, proc_df):
    raw_cols   = raw_col_names()
    delta_cols = delta_col_names()
    # cache: (case, run, ratio) -> raw_feats
    raw_cache = {}
    for row in proc_df.itertuples(index=False):
        cid, rid = int(row.case), int(row.run)
        cut_start = int(row.idx_noload_end)   # cutting region start
        ...
        # cutting region length = min signal length from cut_start
        cut_len = min(len(a) - cut_start for a in arrays.values())
        if cut_len <= 0: continue
        for ratio in INPUT_RATIOS:
            end_offset = max(1, int(np.ceil(cut_len * ratio / 100.0)))
            ...
            raw_cache[(cid, rid, ratio)] = feats
    # first-available-run reference per (case, ratio)
    first_cache = {}
    for ratio in INPUT_RATIOS:
        for cid in proc_df["case"].unique():
            ...
            for r in cd.itertuples(index=False):
                ...
                if (cid, rid, ratio) in raw_cache:
                    first_cache[(cid, ratio)] = raw_cache[(cid, rid, ratio)]
                    break
    rows = []
    for row in proc_df.itertuples(index=False):
        cid, rid = int(row.case), int(row.run)
        ...
        for ratio in INPUT_RATIOS:
            rf = raw_cache.get((cid, rid, ratio))
            if rf is None: continue
            ref = first_cache.get((cid, ratio))
            if ref is None: continue
            delta_feats = rf - ref
            rec = {"case_id": cid, "run": rid, "input_ratio": float(ratio), "VB": vb}
            ...
            rows.append(rec)
    return pd.DataFrame(rows).sort_values(["input_ratio","case_id","run"]).reset_index(drop=True)
...
def build_gru_sequences(feature_df, delta_cols, ratio):
    subset = feature_df[feature_df["input_ratio"]==float(ratio)].copy()
    cases = {}
...
def run_locv(feature_df, all_feat, delta_cols, ratio, model_name, device):
    subset = feature_df[feature_df["input_ratio"]==float(ratio)].copy()
    results = []
    active_cases = sorted(subset["case_id"].unique())

    if model_name == "feature_gru":
        case_data = build_gru_sequences(feature_df, delta_cols, ratio)
        ...
        for tc in active_cases:
            ...
            results.append({"input_ratio": ratio, "model": model_name, "seed": SEED,
                            "target_case": tc, "n_runs": len(y_true), **m})
        return results
    ...
    for tc in active_cases:
        ...
        Xtr = tr_df[all_feat].to_numpy(dtype=np.float64)
        ytr = tr_df["VB"].to_numpy(dtype=np.float64)
        Xte = te_df[all_feat].to_numpy(dtype=np.float64)
        ...
        results.append({"input_ratio": ratio, "model": model_name, "seed": SEED,
                        "target_case": tc, "n_runs": len(yte), **m})
    return results

def main():
    ts = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    out = ROOT/"experiments"/"executions"/"H4"/"S3"/"T4"/f"{ts}_H4_S3_T4_{TOPIC}"
    ...
    feat_df = build_feature_matrix(sig, proc_merged)
    raw_cols   = raw_col_names()
    delta_cols = delta_col_names()
    all_feat   = raw_cols + delta_cols + META_COLS
    log(f"Feature matrix: {len(feat_df)} rows  ({len(feat_df)//len(INPUT_RATIOS)} runs × {len(INPUT_RATIOS)} ratios), {len(all_feat)} features")

    (out/"configs"/f"{PREFIX_LABEL}_input_config.json").write_text(
        json.dumps({"experiment": {"id": f"{ts}_{PREFIX_LABEL}",
                                   "context": "docs/agent_context/task_context/H4_S3_T4_context.md"},
                    "segment": "Entry+Steady+Exit (No-load excluded)",
                    "input_ratios": INPUT_RATIOS,
                    "features": "Raw(24)+Delta(24)+Meta(3)",
    ...
    for mn in TABULAR_MODELS + ["feature_gru"]:
        ...
        for ratio in INPUT_RATIOS:
            try:
                all_results.extend(run_locv(feat_df, all_feat, delta_cols, ratio, mn, device))
            except Exception as e:
                log(f"  ERROR ratio={ratio} {e}")

    mdf = pd.DataFrame(all_results)
    mdf.to_csv(out/"metrics"/f"{PREFIX_LABEL}_locv_per_case.csv", index=False)
    agg = (mdf.groupby(["input_ratio","model"])
           .agg(mean_rmse=("rmse","mean"), mean_r2=("r2","mean"), mean_mae=("mae","mean"),
                n_folds=("target_case","count"))
           .reset_index().sort_values(["model","input_ratio"]))
    agg.to_csv(out/"analysis"/f"{PREFIX_LABEL}_locv_aggregated.csv", index=False)
    mdf[mdf["input_ratio"]==100.0].to_csv(out/"analysis"/f"{PREFIX_LABEL}_100pct_per_case.csv", index=False)

    log("\n=== SUMMARY at 100% input ratio ===")
    s100 = agg[agg["input_ratio"]==100.0][["model","mean_rmse","mean_r2","mean_mae"]]
    log("\n" + s100.sort_values("mean_rmse").to_string(index=False))
    log("\n=== RMSE pivot by input ratio ===")
    log("\n" + agg.pivot(index="input_ratio", columns="model", values="mean_rmse").to_string())

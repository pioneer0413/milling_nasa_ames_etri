#!/usr/bin/env python3
"""H16_S2: Uniform 4-stat (mean, rms, peak2peak, std) on smcAC + smcDC, Input ratio sweep.

Feature design:
  smcAC  → mean, rms, peak2peak (max-min), std  — 4 features
  smcDC  → mean, rms, peak2peak (max-min), std  — 4 features
  Delta: (current_run - first_run) × 8 + meta (DOC, feed, material) = 11-dim total
...
# smcAC and smcDC each get 4 stats → 8 delta + 3 meta = 11 total
SENSORS_USED  = ["smcAC", "smcDC"]
STAT_NAMES    = ["mean", "rms", "p2p", "std"]
FEATURE_NAMES = [f"{s}_{st}" for s in SENSORS_USED for st in STAT_NAMES]  # 8 names
FEATURE_COLS  = [f"d_{n}" for n in FEATURE_NAMES] + META_FEATURES          # 11 total
...
def extract_4stat(arr: np.ndarray) -> np.ndarray:
    """[mean, rms, peak2peak, std] for a 1-D signal array."""
    if len(arr) == 0:
    ...
    return np.array([mean, rms, p2p, std], dtype=np.float32)
...
def build_feature_cache(
    ...
    """Pre-compute 8-dim features for all (case, run, ratio) combinations."""
    ...
    for row in proc_clean.itertuples(index=False):
        ...
        for r in RATIOS:
            ...
            feat   = np.concatenate([
                extract_4stat(ac[:cutoff]),
                extract_4stat(dc[:cutoff]),
            ])
...
def main() -> None:
    ...
    exp_dir = (
        ROOT / "experiments" / "executions" / "H16" / "S2"
        / f"{ts}_uniform_4stat_ratio_sweep"
    )
    ...
    log("=== H16_S2: Uniform 4-stat (mean/rms/p2p/std) on smcAC + smcDC ===")
    log(f"Features: {FEATURE_NAMES}")
    log(f"Total dim: {len(FEATURE_COLS)} (8 delta + 3 meta)")
    ...
    def make_xgb(seed):  return XGBRegressor(**XGB_CFG, random_state=seed)
    def make_svr(_seed): return Pipeline([("scaler", StandardScaler()), ("svr", SVR(**SVR_CFG))])
    def make_rf(seed):   return RandomForestRegressor(**RF_CFG, random_state=seed)
    ...
    for model_name, make_fn in [("XGBoost", make_xgb), ("SVR", make_svr), ("RF", make_rf)]:
        ...
        log(f"  {model_name} best: ratio={best_r}%  RMSE={best_rmse:.6f}"
            f"  Δbaseline={best_rmse-XGB_BEST:+.6f}")
        log("")
    ...
    for model_name, res in all_results.items():
        for r, v in res.items():
            rows.append({
                "model": model_name, "ratio": r,
                "rmse_mean": v["mean"], "rmse_std": v["std"],
                **{f"seed{s}": v["seeds"][i] for i, s in enumerate(SEEDS)},
            })
    pd.DataFrame(rows).to_csv(exp_dir / "data" / "results.csv", index=False)

    # ── Summary ───────────────────────────────────────────────────────────────
    log("=== SUMMARY ===")
    log(f"{'Ratio':>6}  {'XGB_RMSE':>10}  {'SVR_RMSE':>10}  {'RF_RMSE':>10}")
    log("-" * 46)
    for r in RATIOS:
        xr = all_results["XGBoost"][r]["mean"]
        sr = all_results["SVR"][r]["mean"]
        fr = all_results["RF"][r]["mean"]
        xs = "★" if r == min(all_results["XGBoost"], key=lambda x: all_results["XGBoost"][x]["mean"]) else " "
        ss = "★" if r == min(all_results["SVR"],     key=lambda x: all_results["SVR"][x]["mean"])     else " "
        fs = "★" if r == min(all_results["RF"],      key=lambda x: all_results["RF"][x]["mean"])      else " "
        log(f"{r:>5}%  {xs}{xr:.6f}  {ss}{sr:.6f}  {fs}{fr:.6f}")
    log("")
    for model_name, res in all_results.items():
        best_r    = min(res, key=lambda r: res[r]["mean"])
        best_rmse = res[best_r]["mean"]
    ...
    for model_name, res in all_results.items():
        ...
        ax.plot(RATIOS, ys, color=colors[model_name], marker=markers[model_name],
    ...
    ax.axhline(XGB_BEST, color="black", linestyle="--", linewidth=1.2,
               label=f"XGB prefix-90% best (H12, {XGB_BEST:.4f})")
    ax.axhline(0.119431, color="gray", linestyle=":", linewidth=1.0,
               label=f"H16_S1 SVR best (0.1194)")
    ax.set_xlabel("Prefix ratio (%)")
    ax.set_ylabel("Observed-VB RMSE (LOCV)")
    ax.set_title(
        "H16.S2: Uniform 4-stat (mean/rms/p2p/std) on smcAC + smcDC\n"
        "XGBoost / SVR / Random Forest — Prefix Ratio Sweep"
    )
